...
**JSON 可选键**（`src/regions_config.py`）：

| 键 | 含义 |
|----|------|
| `mrf_regions` | 每项 `cellZone` / `origin`（或 `"centroid"`）/ `axis` / `omega`；写入 `constant/air/MRFProperties`。`cellZone` 必须属于 `fluid_regions`，否则转换报错。`nonRotatingPatches` 默认含 AMI、`open*`、`*_to_*`；名称含 `impeller` 的 patch 在 `0/air/U` 中设为 `movingWallVelocity` |
| `heat_sources` | `{区域或 zone 名: 总功率 W}`；同区域多键求和；写 `scalarSemiImplicitSource` + **`volumeMode absolute`**（absolute 语义即“选中单元上的总功率”，OpenFOAM 内部按体积摊薄） |
| `materials` | 按区域覆盖物性。固体：`rho` / `Cp` / `kappa` / `molWeight`；流体 `air`：`mu` / `Pr` / `Cp` / `molWeight`。未覆盖项用内置默认（空气 / 铝） |
| `external_convection` | `{"patches": [正则...], "Ta": 300, "h": 8}`；命中 patch 的 T 写 `externalWallHeatFluxTemperature`（`mode coefficient`，`kappaMethod` 按区域类型取 `fluidThermo`/`solidThermo`） |
| `g` / `gravity` | 重力矢量 → `constant/g`（默认 `(0 0 -9.81)`） |
| `initial_conditions` | `{"T": ..., "p": ...}` 全局初值（默认 300 K / 101325 Pa） |
| `n_procs` / `parallel` | MPI 核数，统一写进各级 `decomposeParDict` 与 `Allrun -np`（默认 8） |
| `endTime` / `writeInterval` / `purgeWrite` | `controlDict` 覆盖（默认 500 / 50 / 0） |

**0/ 场边界条件生成规则**（`src/cht_case.py::_field_*`）：

1. constraint patch（`symmetryPlane` / `empty` / `wedge` / `cyclic`，取自
   polyMesh patch 类型）→ 场写同名类型；
2. `cyclicAMI` patch → `type cyclicAMI;`；
3. `mappedWall` 耦合界面 → T 用
   `compressible::turbulentTemperatureRadCoupledMixed`（`Tnbr T`，
   `kappaMethod` 按区域类型），U `noSlip`，p_rgh `fixedFluxPressure`；
4. 开口（patch 名 `open*`，或 CGNS BCType 为 `BCInflow` / `BCOutflow` /
   `BCFarfield` / `BCExtrapolate` 等——CGNS 类型随 patch plan 透传，
   见 `src/cht_direct.py::is_opening_bc_type`）→
   `prghTotalPressure` + `pressureInletOutletVelocity` + T `inletOutlet`；
   按 BC 类型识别出的开口会在控制台与 `setup_report.json` 的
   `warnings` 中提示（默认取 p0/T0，需人工校核）；
5. `external_convection` 命中的外壁面 → `externalWallHeatFluxTemperature`；
6. 其余壁面 → U `noSlip`、T `zeroGradient`（绝热）、p/p_rgh `calculated` /
   `fixedFluxPressure`。

**求解设置**（自动生成）：

- `SIMPLE`：`momentumPredictor true`、`nNonOrthogonalCorrectors 1`、
  显式 `residualControl`（流体 `p_rgh 1e-5 / U 1e-5 / h 1e-6`，固体 `h 1e-6`）。
- `div(phi,U/h/K)` 一阶 `bounded Gauss upwind` 起步；`fvSchemes` 内附
  二阶 `linearUpwind` 注释模板。
- 流体 fvOptions：`limitTemperature`（200–500 K）+ `limitVelocity`（100）。
...
跳过 mono 拼接与 `splitMeshRegions`。`Allrun` 做
`decomposePar -allRegions` → 并行求解 → `reconstructPar(Mesh)`。

> 旧的 `--cht` 两阶段模式（mono + `splitMeshRegions`）已移除：其界面
> patch 在 split 后仍是 `wall`，0.orig 的 T 为 `zeroGradient`，各区域
> 无法共轭耦合；且 AMI patch 的场类型依赖 BC 名称启发式，容易在
> createPatch 后与场类型失配。
...
8. **不读取 CGNS BCDataSet**：`BC_t` 节点携带的 Dirichlet/Neumann 数值
   （进口速度、壁温、热流等）不会被搬运；开口统一按总压开口模板生成，
   数值取 JSON 的 `initial_conditions`，需在 OpenFOAM 侧校核。
9. **湍流模型固定 laminar**（脚手架取向）：未生成 k/omega/epsilon/nut/
   alphat 场文件；切换 RAS 模型时需手工补 0/ 场与壁面函数。
10. **同一 OpenFOAM 区域内的固-固界面不自动连通**：`stitch` 类耦合目前
    保留为普通 wall patch（区域内隔热）；跨区域的固-固为 `mappedWall`，
    不受影响。
